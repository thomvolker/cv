...
print_publications <- function(rmd){
  invisible(rmarkdown::render(rmd, "pdf_document", output_file = "~/Documents/CV/my_publications.pdf",
                              quiet = TRUE))
  
  publi <- pdftools::pdf_text("~/Documents/CV/my_publications.pdf") %>% 
    gsub("\n   ", " ", .) %>% 
    strsplit(split = "\n") %>%
    unlist(recursive = F)
  
  index <- purrr::map_dbl(publi, function(x) 
    if (grepl("\\(n.d.\\).", x) | grepl("\\(accepted\\).", x)) {return(9999)}
    else {
      stringr::str_extract(x, pattern = "\\([:digit:]{4,4}\\).") %>%
      stringr::str_extract(pattern = "[:digit:]{4,4}") %>% as.numeric
    })
  
  cat(paste0(publi[order(index, decreasing = T)], sep = "\n\n"))
